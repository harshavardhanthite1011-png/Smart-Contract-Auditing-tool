...
export class Contract {
  ...
  _test_0(b_0) { __compactRuntime.assert(!b_0, 'Error'); return []; }
}
...
export const pureCircuits = {
  test: (...args_0) => {
    if (args_0.length !== 1) {
      throw new __compactRuntime.CompactError(`test: expected 1 argument (as invoked from Typescript), received ${args_0.length}`);
    }
    const b_0 = args_0[0];
    if (!(typeof(b_0) === 'boolean')) {
      __compactRuntime.typeError('test',
                                 'argument 1',
                                 'scratch.compact line 4 char 1',
                                 'Boolean',
                                 b_0)
    }
    return _dummyContract._test_0(b_0);
  }
};
